...
static int tstki;
static char *titlestack[TITLESTACKSIZE];
...
{
	float winx = win.hborderpx + x * win.cw,
	    winy = win.vborderpx + y * win.ch, xp, yp;
...
xsettitle(char *p, int pop)
{
	...
	/* Store the same default title that an empty OSC title presents. */
	if (!pop && (!p || p[0] == '\0'))
		p = opt_title;
...
run:
	...
	if (!opt_title)
		opt_title = (opt_line || !opt_cmd) ? "sc" : opt_cmd[0];
